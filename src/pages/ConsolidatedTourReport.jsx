...
const fetchConsolidatedTourReport = async () => {
  try{
   ...
   _fetch('consolidatedtourreport',payload,false,token).then(res => {
    ...
    console.error('Error fetching Consolidated Tour Report',error)
  }
}


const ExcelReportTourConsolidated = async (data) => {
...
const createSheet = (sheetName,headers,data) => {
  ...
  let titleRow;
  titleRow = sheet.addRow([`Consolidated Tour Compliance Report - ${fromDate} to ${toDate}`]);
...
if(Array.isArray(data) && data.length > 0){
  ...
  createSheet("Consolidated Tour Report",customHeaders,data);
} else {
...
saveAs(blob,`ConsolidatedTourReport_${new Date().toISOString().split('T')[0]}.xlsx`);
}
...
export default ConsolidatedTourReport